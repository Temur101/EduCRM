...
const loadHelperData = async () => {
  const { data: cData } = await supabase.from('courses').select('id, name').eq('status', 'Active');
  courses.value = cData || [];
  const { data: tData } = await supabase.from('teachers').select('id, name').eq('status', 'Active');
  teachers.value = tData || [];
};

const loadOtherGroups = async () => {
  try {
    const { data } = await supabase.from('groups').select('id, name').neq('id', groupId).eq('status', 'Active');
...
const deleteGroupAction = async () => {
  ...
  try {
    ...
    if (deleteOption.value === 'moveAnother') {
      ...
      const { data: groupStudents } = await supabase.from('students').select('*').eq('group_id', groupId);
      if (groupStudents?.length) {
        for (const student of groupStudents) {
          await supabase.from('archives').insert([{
            type: 'student',
            original_id: String(student.id),
            data: student,
...
const loadGroupData = async () => {
  ...
  try {
    const { data: groupData, error: groupError } = await supabase
      .from('groups')
      .select('*, courses(name, price), teachers(name)')
      .eq('id', groupId)
      .single();
