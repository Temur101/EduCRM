...
const loadData = async () => {
  ...
  try {
    ...
    let gQuery = supabase.from('groups').select('id, name').eq('status', 'Active').eq('is_deleted', false);
...
const deleteStudent = async () => {
  ...
  try {
    ...
    alert('Студентни ўчиришда хатолик юз берди: ' + (e.message || 'Номаълум хатолик'));
  } finally {
