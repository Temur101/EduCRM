...
const deleteGroup = async () => {
  ...
  try {
    ...
    if (deleteOption.value === 'moveAnother') {
      ...
      // Delete students
      const { error: delStudError } = await supabase.from('students').delete().eq('group_id', id);
      if (delStudError) throw delStudError;
    }

    // 2. Archive the group itself
    const groupToArchive = groups.value.find(g => g.id === id);
    if (groupToArchive) {
      const { error: archGroupErr } = await supabase.from('archives').insert([{
        type: 'group',
        original_id: String(id),
        data: groupToArchive,
        archived_at: new Date().toISOString()
      }]);
      if (archGroupErr) throw archGroupErr;
    }

    // 3. Delete group
    const { error: deleteGroupErr } = await supabase.from('groups').delete().eq('id', id);
