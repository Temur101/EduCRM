...
const syncExistingTeachers = async () => {
  try {
    ...
    const { data: allTeachers } = await supabase.from('teachers').select('*').eq('is_deleted', false);
    if (!allTeachers || allTeachers.length === 0) return;

    // 2. Fetch all staff emails
    const { data: allStaff } = await supabase.from('staff').select('email');
    const staffEmails = new Set((allStaff || []).map(s => s.email));

    // 3. Find teachers that are NOT in staff
    const missingTeachers = allTeachers.filter(t => t.email && !staffEmails.has(t.email));

    if (missingTeachers.length > 0) {
      console.log(`Syncing ${missingTeachers.length} missing teachers to staff...`);
      const newStaffEntries = missingTeachers.map(t => {
        const entry = {
          name: t.name,
          email: t.email,
          phone: t.phone,
      ...
      const { error: syncError } = await supabase.from('staff').insert(newStaffEntries);
      if (syncError) {
        console.error('Supabase Sync Details:', syncError.message, syncError.details, syncError.hint);
        throw syncError;
      }
      return true;
    }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (staffForm.role === 'teacher') {
      ...
      // Try to find teacher by email to sync
      ...
      if (existingTeacher) {
        await supabase.from('teachers').update(teacherObj).eq('id', existingTeacher.id);
      } else {
        await supabase.from('teachers').insert([teacherObj]);
      }
        ...
        <div class="modal-box">
          ...
          <div class="modal-body">
            ...
            <div class="form-group" v-if="staffForm.role === 'teacher'" style="grid-column: 1 / -1; margin-top: 1rem;">
              <label style="font-size: 1rem; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.5rem;">
                <Users :size="18" /> Guruhlarni biriktirish
              </label>
